...
final class AccountWizardController: NSWindowController {
    ...
    /// sign-in, NewEditSignIn). With `requestPassword` a password account
    /// being edited opens on the identity page asking for its password,
    /// the reason (`authRequired`, `authFailed`) saying why
    /// (`WizardController.requestPassword`). `confirmTrust` asks before a
    /// server's certificate is pinned. `onDone` runs after account.add /
    ...
    static func present(
        ...
        requestPassword: ErrorCode? = nil, confirmTrust: @escaping WizardConfirmTrust,
        onDone: @escaping @MainActor (AccountID, AccountConfig) -> Void
    ) {
        let c = AccountWizardController(client: client, editing: editing, signIn: signIn, requestPassword: requestPassword)
        c.onDone = onDone
    ...
    private init(client: RPCClient, editing: Account?, signIn: Bool, requestPassword: ErrorCode?) {
        let wizard = WizardController(client: client, editing: editing, signIn: signIn)
        // Before the pages are built, so the wizard opens on the identity.
        if let reason = requestPassword {
            wizard.requestPassword(reason: reason)
        }
        self.wizard = wizard
