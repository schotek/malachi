...
public actor DaemonSupervisor {
    ...
    public struct Launch: Sendable {
        ...
        public init(executable: URL, socket: String, config: String, store: String) {
    ...
    private func spawn(_ l: Launch) throws {
        ...
        // No Secret Service on macOS in this phase: the daemon starts, and
        // adding an account with a password fails with keyringError.
        if env["MALACHI_KEYRING"] == nil {
            env["MALACHI_KEYRING"] = "none"
        }
        p.environment = env
